...
void lv_poly_line::add_line()
{
    ...
    PM_LOG_INFO("line: %p, parent: %p", line, parent);
}
...
void lv_poly_line::start()
{
    ...
    {
        PM_LOG_INFO("current_index[%d] >= poly_line.size()[%d]", current_index, poly_line.size());
        add_line();
        refresh_line();
    }

    PM_LOG_INFO("show poly_line[%d]", current_index);
...
void lv_poly_line::append(const lv_point_t* point)
{
    if (poly_line.size() == 0)
    {
        PM_LOG_WARN("poly_line.size() = 0");
        return;
    }

    PM_LOG_INFO("poly_line[%d]: (%d, %d)", current_index, point->x, point->y);
...
void lv_poly_line::append_to_end(const lv_point_t* point)
{
    ...
    PM_LOG_INFO("end_line: (%d, %d), size = %d", point->x, point->y, single_line->points.size());
...
void lv_poly_line::stop()
{
    ...
    PM_LOG_INFO("poly_line[%d] size = %d", current_index, single_line->points.size());
    current_index++;
}
...
void lv_poly_line::reset()
{
    current_index = 0;
    size_t size = poly_line.size();
    PM_LOG_INFO("poly_line.size() = %d", size);
    PM_LOG_INFO("reset current_index");
...
lv_poly_line::single_line_t* lv_poly_line::get_end_line()
{
    ...
    if (size == 0)
    {
        PM_LOG_ERROR("NOT found");
        return nullptr;
    }
    else
    {
        PM_LOG_INFO("end_line index = %d", current_index - 1);
    }
...
bool lv_poly_line::get_end_point(lv_point_t* point)
{
    ...
    if (single_line == nullptr)
    {
        PM_LOG_ERROR("end line NOT found");
        return false;
    }

    if (single_line->points.size() == 0)
    {
        PM_LOG_ERROR("single_line->points is EMPTY");
        return false;
    }
